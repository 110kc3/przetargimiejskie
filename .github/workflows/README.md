...
|---|---|---|---|---|
| 1 | `refresh.yml` | Refresh auction data | daily 04:00 UTC · push to `main` touching pipeline code · manual | The main crawl. Matrix = one job per hosted-safe city from the registry (`pipeline/src/cities/index.js`), `fail-fast: false`, so one broken city never blocks the rest; adapters marked `needsResidentialEgress` are deferred until the restricted proxy is deployed. Each job: crawl → `sanity-check.js` gate → geoportal enrich → commit only `data/<city>/` (rebase-retry push). A separate one-per-run job refreshes AMW and validates the complete PKP/AMW ledger; PKP stays on last-good data under the expiring stale-only health warning while Azure egress is blocked. Then: `index` rebuilds city `data/index.json`; `triage` files/updates/closes per-city `[city-broken]` issues. |
