...
| 6 | `security.yml` | Security | push to `main` · every PR (including stacked reviews) · Mondays 07:00 UTC · manual | Enforces GitHub-hosted runners, runs CodeQL, requires zero open CodeQL alerts on `main` for non-PR runs, and runs blocking Trivy dependency/secret scanning at medium severity or above. Findings are also uploaded to the Security tab. |
