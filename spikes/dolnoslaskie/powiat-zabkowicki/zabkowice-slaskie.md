...
> **Status:** spike LIVE — 2026-07-09. VERDICT: BUILD (Low effort). **Built + registered 2026-07-11** (14/14 parse test). ⚠️ Build correction: the "Informacja o wyniku przetargu" result PDFs are **SCANNED** (KONICA MINOLTA bizhub, no text layer — `pdftotext` yields 0 chars), NOT born-digital as this spike assumed; the adapter OCRs them (`core/ocr-pdf.js`, `-l pol`), and achieved-price history backfills over several bounded CI runs. Announcements need no OCR (Logonet inline HTML "Szczegóły" table).
