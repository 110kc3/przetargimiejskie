...
test('single-city dispatch rejects unknown and residential-egress cities', () => {
  ...
  assert.throws(() => buildRefreshMatrix(cities, 'pszczyna'), /requires operator-managed residential egress/);
  assert.throws(() => buildRefreshMatrix(cities, 'raciborz'), /requires operator-managed residential egress/);
});
