...
> **Status:** spike LIVE — 2026-07-09. VERDICT: BUILD (Low–Medium effort). **Built + registered 2026-07-11** (parse test groundtruthed on live wolow.pl fixtures). ⚠️ Build correction — see §4: the gmina publishes **NO** achieved-price stream (the "Prawików 84 200 zł" figure this spike cited is a *county* result, a different JST), so the shipped adapter is **unsold-only**.
...
- **Achieved-price stream:** ~~YES~~ → **CORRECTED at build (2026-07-11): NO gmina hammer-price stream.** The "Prawików wylicytowano 84 200,00 zł" result this spike cited is a **county** document (`bip.powiatwolowski.pl`, dz. nr 223 AM-1 — Starostwo Powiatowe w Wołowie, a *different* JST), not a gmina `wolow.pl` notice; no gmina surface publishes `cena osiągnięta`/`nabywca`. The shipped adapter's outcome stream is therefore **unsold-only**: a round K+1 announcement for the same subject (address key for flats/houses/commercial; dzialka_nr+obręb for land) proves round K failed to sell (ADAPTER-GUIDE §5.5), and `crawlResultDocs()` forwards round K's own real text (address/area/`cena wywoławcza`/date — never a hammer price). Announcements still carry `cena wywoławcza`.
