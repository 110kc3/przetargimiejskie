...
|---|---|---|---|---|
| 1 | `refresh.yml` | Refresh auction data | daily 04:00 UTC · push to `main` touching pipeline code · manual | The main crawl. Matrix = one read-only job per hosted-safe city from the registry (`pipeline/src/cities/index.js`), `fail-fast: false`, so one broken city never blocks the rest. Racibórz/Pszczyna join the restricted tailnet only when all three egress secrets exist. Crawl jobs upload hashed allowlisted artifacts; a separate write-enabled publisher validates paths, types, sizes, hashes and JSON and reruns sanity before committing. Providers use the same separation and retain PKP's fresh-runner retries. Then `index` rebuilds `data/index.json`; `triage` files/updates/closes per-city `[city-broken]` issues. |
...
| 7 | `backfill.yml` | Backfill (manual) | manual only | Full parallel refresh with a 350-min per-city timeout and unbounded crawlers — primes cold OCR/text caches for new cities. Uses the same restricted-egress and read-only-crawler/validated-publisher boundaries as workflow 1, and shares its concurrency lock. |
